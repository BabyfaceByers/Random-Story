...
def save():
    global location,playerlocx,playerlocy,youritems
    precodedyouritems=""
    codedlocation=(base64.b64encode(bytes(location, 'utf-8'))).decode("utf-8")
    codedlocation=(base64.b64encode(bytes(codedlocation, 'utf-8'))).decode("utf-8")
    codedplayerlocx=(base64.b64encode(bytes(str(playerlocx), 'utf-8'))).decode("utf-8")
    codedplayerlocx=(base64.b64encode(bytes(str(codedplayerlocx), 'utf-8'))).decode("utf-8")
    codedplayerlocy=(base64.b64encode(bytes(str(playerlocy), 'utf-8'))).decode("utf-8")
    codedplayerlocy=(base64.b64encode(bytes(str(codedplayerlocy), 'utf-8'))).decode("utf-8")
    for item in youritems:
        precodedyouritems=precodedyouritems+item+","
    codedyouritems=(base64.b64encode(bytes(precodedyouritems, 'utf-8'))).decode("utf-8")
    codedyouritems=(base64.b64encode(bytes(codedyouritems, 'utf-8'))).decode("utf-8")

    file=open("Savegame.OTCESF","w+")
    file.write(codedlocation + "\n" + codedplayerlocx + "\n" + codedplayerlocy + "\n" + codedyouritems)
def load():
    global location,playerlocx,playerlocy,youritems
    loadarray=[]
    file=open("Savegame.OTCESF","r")
    
    for line in file:
        loadarray.append(line.rstrip("\n"))
    file.close()
    codedlocation=(base64.b64decode(loadarray[0])).decode("utf-8")
    codedplayerlocx=(base64.b64decode(loadarray[1])).decode("utf-8")
    codedplayerlocy=(base64.b64decode(loadarray[2])).decode("utf-8")
    location=(base64.b64decode(codedlocation)).decode("utf-8")
    playerlocx=int((base64.b64decode(codedplayerlocx)).decode("utf-8"))
    playerlocy=int((base64.b64decode(codedplayerlocy)).decode("utf-8"))
    try:
        codedyouritems=(base64.b64decode(loadarray[3])).decode("utf-8")
        youritems=(base64.b64decode(codedyouritems)).decode("utf-8")
        youritems=youritems.split(",")
        youritems.pop(-1)
        print(youritems)
    except:
        youritems=[]
...
def cutscene(number):
    ...
    if number==1:
        ...
        oak=pygame.image.load("entities/Oak.png")
        for y in range(0,3):
            if y==0:
                for x in range(0,4):
                    pygame.time.Clock().tick(30)
                    resetdisplay()
                    gameDisplay.blit(((pygame.transform.scale(oak,(32,32)))),(512-x*8,playerlocy))
                    pygame.display.update()
            elif y==1:
                for x in range(0,48):
                    pygame.time.Clock().tick(30)
                    resetdisplay(True)
                    playerlocx=448-(8*x)-8
                    print(playerlocx)
                    gameDisplay.blit(((pygame.transform.scale(oak,(32,32)))),(playerlocx+32,playerlocy))
                    gameDisplay.blit(((pygame.transform.scale(player,(32,32)))),(playerlocx,playerlocy))
                    pygame.display.update()
            elif y==2:
                while not EnterPress:
                    pygame.time.Clock().tick(30)
                    ...
                    resetdisplay(True)
                    gameDisplay.blit(((pygame.transform.scale(oak,(32,32)))),(playerlocx+32,playerlocy))
                    textbox("Oak: There's a time and place for everything, But not now! Oops, I didn't mean to kill him!")
                    pygame.display.update()
...
def removeitem(itm):
    youritems.remove(youritems[itm])
...
def main():
    ...
    while True:
        ...
        if checkhaveitem("Golden Book")==False and location=="cave":
            gameDisplay.blit((pygame.transform.scale(pygame.image.load("entities/GoldenBook.png"),(32,32))),(Gridx[9],Gridy[4]))
            if (playerlocinfo[0]<Gridx[10] or playerlocinfo[2]<Gridx[10]) and (playerlocinfo[0]>Gridx[10-1] or playerlocinfo[2]>Gridx[10-1]) and (playerlocinfo[1]<Gridy[5] or playerlocinfo[3]<Gridy[5]) and (playerlocinfo[1]>Gridy[5-1] or playerlocinfo[3]>Gridy[5-1]):
                print("In Spot")
                if EnterPress:
                    EnterPress=False
                    giveitem("Golden Book")
                    while not EnterPress:
                        textbox("You got the Golden Book!\n Added the Golden Book to your inventory")
                        gameDisplay.blit((pygame.transform.scale(player,(32,32))),(playerlocx,playerlocy))
                        for event in pygame.event.get():
                            if event.type==pygame.QUIT:
                                quitgame()
                            if event.type==pygame.KEYDOWN:
                                if event.key == pygame.K_RETURN:
                                    EnterPress=True
                        pygame.display.update()
        if location=="downhill" and playerlocx<=448 and 32<=playerlocy<=96:
            cutscene(2)
            GameOver((128,64,0))
...
def startscreen():
    ...
    while startmenuopen:
        while not started:
            ...
            pygame.draw.rect(gameDisplay, (0,0,0), ((0,484),(512,28)))
            blit_text(gameDisplay, "Press Enter to Start", (363, 486), pygame.font.SysFont('arial', 20), (255,255,255))
            blit_text(gameDisplay, "2.0.2", (6, 489), pygame.font.SysFont('arial', 17), (255,255,255))
            pygame.display.update()
            for event in pygame.event.get():
                if event.type==pygame.QUIT:
                    pygame.quit()
        ...
        menuText1=menuText.render("Load",True,(0,0,0))
        textRect1=menuText1.get_rect()
        textRect1.center=((392+(104/2),(16+(32/2))))

        menuText2=menuText.render("New Game",True,(0,0,0))
        textRect2=menuText2.get_rect()
        textRect2.center=((392+(104/2),(52+(32/2))))
        ...
        if menuSelection==1:
            pygame.draw.rect(gameDisplay, (0,0,0), ((16,16),(480,32)),5)
        elif menuSelection==2:
            pygame.draw.rect(gameDisplay, (0,0,0), ((16,52),(480,32)),5)
        elif menuSelection==3:
            pygame.draw.rect(gameDisplay, (0,0,0), ((16,88),(480,32)),5)
        ...
        pygame.display.update()
def GameOver(color=(255,255,255),noplayer=True):
    global location,playerlocx,playerlocy,youritems,wHeld,aHeld,sHeld,dHeld
    EnterPress=False
    Dead=True
    fade = pygame.Surface((512, 512))
    fade.fill((0,0,0))
    menuSelection=0
    for alpha in range(0, 50):
        fade.set_alpha(alpha*6)
        resetdisplay(noplayer)
        pygame.draw.rect(fade, (0,0,0), ((0,0),(512,512)))
        pygame.draw.rect(fade, (255,255,255), ((4,4),(504,504)),5)
        pygame.draw.rect(fade, (100,100,100), ((12,12),(488,488)))
        gameDisplay.blit(fade, (0,0))
        pygame.display.update()
    while Dead:
        for event in pygame.event.get():
        ...
        if menuSelection == 3:
            menuSelection = 1
        pygame.draw.rect(gameDisplay, (0,0,0), ((0,0),(512,512)))
        pygame.draw.rect(gameDisplay, (255,255,255), ((4,4),(504,504)),5)
        pygame.draw.rect(gameDisplay, (100,100,100), ((12,12),(488,488)))

        menuText=pygame.font.SysFont("arial",int(32/2))
        titleText=pygame.font.SysFont("arial",32)

        menuText0=titleText.render("Game Over",True,color)
        textRect0=menuText0.get_rect()
        textRect0.center=((512/2),(66+32))
            
        menuText1=menuText.render("Load Save",True,(0,0,0))
        textRect1=menuText1.get_rect()
        textRect1.center=((512/2),(128+(32/2)))

        menuText2=menuText.render("Quit",True,(0,0,0))
        textRect2=menuText2.get_rect()
        textRect2.center=((512/2),(164+(32/2)))

        gameDisplay.blit(menuText0,textRect0)
        gameDisplay.blit(menuText1,textRect1)
        gameDisplay.blit(menuText2,textRect2)
        
        if menuSelection==1:
            pygame.draw.rect(gameDisplay, (0,0,0), ((192,128),(128,32)),5)
        elif menuSelection==2:
            pygame.draw.rect(gameDisplay, (0,0,0), ((192,164),(128,32)),5)
